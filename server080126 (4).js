...
app.post('/api/diary/day', tgAuth, async (req, res) => {
  try {
    ...
    const score    = Number((body.feeling_score ?? 0) || 0);

    // ВАЖНО: если поле НЕ пришло или пришло null — НЕ затираем сохранённые значения на сервере.
    const hasSE     = Object.prototype.hasOwnProperty.call(body, 'side_effects');
    const effects   = hasSE ? (body.side_effects === null ? null : body.side_effects) : null;

    const hasComment = Object.prototype.hasOwnProperty.call(body, 'comment');
    const comment    = hasComment ? (body.comment === null ? null : String(body.comment)) : null;
    ...
    const sql = `
      WITH upd AS (
        UPDATE diary
           SET feeling_score = $3,
               side_effects  = CASE
                                WHEN $4::jsonb IS NULL THEN side_effects
                                ELSE COALESCE($4::jsonb, '{}'::jsonb)
                              END,
               comment       = CASE
                                WHEN $5 IS NULL THEN comment
                                ELSE $5
                              END
         WHERE patient_id = $1 AND at_date = $2::date
         RETURNING id
